...
def softmax_loss(Z, y):
    ...
    ### BEGIN YOUR CODE
    b_size = Z.shape[0]
    return (-Z[np.arange(b_size), y] + np.log(np.exp(Z).sum(axis=1))).mean()
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for iter in range(total_iters):
        ...
        Z_b = np.exp(X_b @ theta)
        Z_b = Z_b / Z_b.sum(axis=1)[:, np.newaxis]

        Iy_b = np.zeros((batch, theta.shape[1]))
        Iy_b[np.arange(batch), y_b] = 1.
